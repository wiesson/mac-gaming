...
3. For a controller, first pair it in macOS (USB or Bluetooth), then check **Steam > Settings > Controller** in this Windows Steam client and test in game. The [Steam game page](https://store.steampowered.com/app/1371980/No_Rest_for_the_Wicked/) lists Xbox and PlayStation controllers and recommends controller play. Controller passthrough in this Wine profile is not yet verified.
   - A Switch Pro Controller was recognized by macOS and Windows Steam on September 25, 2026, while the game did not respond to it. Steam loaded a Switch Pro gamepad template and reserved XInput slot 0. The tested profile had the per-game Steam Overlay disabled. [Valve documents](https://partner.steamgames.com/doc/features/steam_controller/steam_input_gamepad_emulation_bestpractices?l=german) that its Windows gamepad emulation uses the overlay to hook XInput. After the game and Steam have stopped, test `python3 tools/configure_nrftw.py --app "$HOME/Games/No Rest for the Wicked (Wine).app" --overlay on --apply`, then restart and test the game. This is a diagnostic step, not yet a confirmed fix.
