...
3. For a controller, first pair it in macOS (USB or Bluetooth), then check **Steam > Settings > Controller** in this Windows Steam client and test in game. The [Steam game page](https://store.steampowered.com/app/1371980/No_Rest_for_the_Wicked/) lists Xbox and PlayStation controllers and recommends controller play. Controller passthrough in this Wine profile is not yet verified.
   - On September 25, 2026, a Switch Pro Controller was recognized by macOS and Windows Steam. Wine enumerated it as HID and XInput, and button presses were visible in Wine's `control joy.cpl` test. The game's `Player.log` reported `Joystick connected ("Controller (XBOX 360 For Windows)")`, yet gameplay did not react. Enabling the per-game Steam Overlay and restarting did not fix it, despite [Valve's documentation](https://partner.steamgames.com/doc/features/steam_controller/steam_input_gamepad_emulation_bestpractices?l=german) that Windows Steam Input gamepad emulation uses the overlay to hook XInput. Steam's controller log reported `Controller 0 mapping uses xinput : false` on the latest launch. The next isolated test is **Properties > Controller > Override > Disable Steam Input** for this game, followed by a restart. If that works, Wine's own controller path can be used without Steam's mapping. If it fails, restore Steam Input and test a keyboard/mouse controller layout.
