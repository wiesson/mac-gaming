...
**Tested route:** Windows Steam inside a separate [Sikarugir](https://github.com/Sikarugir-App/Sikarugir) wrapper, Wine 10.0 revision 6, Apple's D3DMetal for Direct3D 11, and MSync. Tested on an M3 Max with macOS 27.0 on September 24–25, 2026. Steam sign-in, game login, gameplay, exit code 0, and Steam Cloud upload were observed. A Switch Pro Controller worked in the game after Steam Input was disabled for this title. The player said performance felt very good; there is no FPS benchmark.
...
3. For the tested Switch Pro Controller, connect it to macOS before launching the game. In **Windows Steam > Library > No Rest for the Wicked > Properties > Controller**, set the per-game override to **Disable Steam Input**. Restart the game after changing it. The player confirmed that controller input then worked in gameplay. Steam saved `UseSteamControllerConfig=0` for app 1371980 in `localconfig.vdf`. This setting is per game; other games can keep their own Steam Input choices. The [Steam game page](https://store.steampowered.com/app/1371980/No_Rest_for_the_Wicked/) lists Xbox and PlayStation controllers, but they were not tested in this Wine profile.
   - During diagnosis on September 25, 2026, macOS and Windows Steam recognized the Switch Pro Controller. Wine enumerated it as HID and XInput, button presses were visible in Wine's `control joy.cpl` test, and the game's `Player.log` reported `Joystick connected ("Controller (XBOX 360 For Windows)")`. With Steam Input enabled, the game did not respond. Enabling the per-game Steam Overlay and restarting did not fix it. Disabling Steam Input for the game and restarting did. The overlay was enabled in the successful session; whether it is needed with Steam Input disabled was not tested.
   - A [Windows user reported](https://steamcommunity.com/app/1371980/discussions/0/4369130369053220505/) the same Switch Pro symptom and solved it with DS4Windows. That tool relies on a Windows virtual controller driver and is not a Wine fix. A [Linux/Proton user reported](https://forum.norestforthewicked.com/t/switch-pro-controller-not-detected-on-linux-proton/25576) a similar symptom and solved missing `/dev/uinput` access; that Linux device and fix do not apply on macOS.
